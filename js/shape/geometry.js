	...
	faceDefs.forEach(
		({
			...
			if (isTriangle) {
				...
				triP2 = new THREE.Vector2(v0ToV2.dot(xAxis), v0ToV2.dot(yAxis));
				...
				uvs = new Float32Array([
					(triP0.x - triMinX) / triRangeX,
					1 - (triP0.y - triMinY) / triRangeY,
					(triP1.x - triMinX) / triRangeX,
					1 - (triP1.y - triMinY) / triRangeY,
					(triP2.x - triMinX) / triRangeX,
					1 - (triP2.y - triMinY) / triRangeY,
				]);
			} else {
				// Square face (base)
				positions = new Float32Array([
			...
			const imgOffsetY =
				faceOffsetY !== undefined ? faceOffsetY : FACE_IMG_OFFSET_Y;
			...
			const textPosMultiplier = getResponsiveTextPosMultiplier(
				window.innerWidth,
			);
