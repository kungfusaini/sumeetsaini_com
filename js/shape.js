...
const startIdleTimer  = () => {
  clearIdleTimer();
  idleTimer = setTimeout(() => { hasInteracted = false; userVel = { x: 0, y: 0 }; }, IDLE_TIMEOUT_MS);
};
...
function buildPyramid() {
  ...
  FACES.forEach(f => {
    const mat  = new THREE.MeshBasicMaterial({ color: f.color, side: THREE.DoubleSide });
...
function onClick(ev, container) {
  const mouse = new THREE.Vector2(
    (ev.clientX / container.clientWidth)  * 2 - 1,
    -(ev.clientY / container.clientHeight) * 2 + 1
  );
...
function init() {
  ...
  container.addEventListener('touchstart', e => onPointerDown(e.touches[0].clientX, e.touches[0].clientY));
...
function bootPyramid() {
  if (!$('#name')?.classList.contains('top')) return setTimeout(bootPyramid, 100);
  init();
}
bootPyramid();
